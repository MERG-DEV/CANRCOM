...
;	include		"..\cbuslib\cbusdefs8r.inc"
	include		"c:\Users\ardma\Documents\Merg\defs\cbusdefs8s.inc"
...
; and proved working but the hardware causes and issue with the trigger window
; created by the pic. A 10nF cap. connected across the CT secondary goes some way
; to relieving the problem. MPS

; Verson 7 will contain re-writtern RalCom decoding section. MPS 
...
MTYP_CANRCOM EQU	d'66'		; definition for now.
...
MODULE_ID	EQU MTYP_CANRCOM	; d'66'			;Till in cbusdefs.
...
; end of bootloader RAM
	...
	CBLOCK 0
		...
		Bflag			; for both hi amd lo address bytes
		Tx1con
		Tx1sidh
		Tx1sidl
		Tx1eidh
...
do_1:
	BTFSS	Dat1,2				; is it hi byte?  ID= 2	(add more here for a consist)
...
do_7:
;	BTFSS		Sflag,0				; Send once flag
;	CALL		sendTXb				; as sendTXa but wait till sent
	CLRF		Flags

	BSF		Sflag,0				; Set Send once flag
...
ch2_1:
	BCF		Nvtemp,1				; don't do Ch 1 again

ch2:
	NOP								; as channel 1 but do once
	CLRF		PIR1					; clear flags
...
;	SUBWF	ID_old,W
;	BZ			do2_1a
;	BCF		Sflag,0				; Clear Send once flag
; do2_1a:
...
set_DDES1:	
	BTFSC	Sflag,0				; Test send RailCom once flag
	...
	MOVLW	d'8'
	MOVWF	Dlc
	BTFSS	Sflag,0				; Test send RailCom once flag
	 CALL		sendTXb
...
;	BRA		no_occ
	...
	MOVF		DNlo,F				; is DNlo = 0
	BNZ		un_occ1
	BRA		un_occ2				; Invalid DN don't send
un_occ1:
	BCF		PIE1,TMR1IE
