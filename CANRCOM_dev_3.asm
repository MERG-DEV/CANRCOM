...
;	include		"..\cbuslib\cbusdefs8v.inc"
...
; end of bootloader RAM
	...
	CBLOCK 0
		...
		Loc_flgs		; Loco Informationtion flags
		Tx1con
		Tx1sidh
		Tx1sidl
		Tx1eidh
...
proc_data:	
	...
	BTFSC	Dgm1,1				; Extended address marker
	 BSF		Dgm2,6
	BTFSC	Dgm1,1
	 BSF		Dgm2,7
...
CV17:
	BTFSS	Bflag,7				; 2nd pass indicator
...
x2Pass:
	BSF		Bflag,7				; Blag bit7 = 1 - 2nd Pass bit
	RETURN
...
;**************************************************************	
	...
	MOVWF	DNhi	
	CALL		eewrite				; Save it to eeprom 0x10
	INCF		EEADR				; Move to next eeprom entry 0x11
	MOVF		RXB0D4,W			; next byte
	MOVWF	DNlo
	CALL		eewrite				; Save it to eeprom
...
unlearn:
	...
	MOVWF	EEADR
	MOVLW	0
	MOVWF	DNhi					; Allocate d'0' to DNhi
	CALL		eewrite				; Save it at eeprom address 0x10
	INCF		EEADR				; Move to next eeprom address
	MOVLW	0
	MOVWF	DNlo					; Allocate d'0' to DNlo
	CALL		eewrite				; Save it at eeprom address 0x11				
...
newid:
	...
	CALL		eeread
	MOVWF	Datmode
	MOVLW	LOW DevNo			; Eeprom address 0x10
	MOVWF	EEADR
	CALL		eeread
	MOVWF	DNhi
	INCF		EEADR				; Move Eeprom address to 0x11	
	CALL		eeread
	MOVWF	DNlo
	MOVLW	LOW NodeID
	MOVWF	EEADR
...
un_occ2:
;***************Don't Send and Reset Unoccupied*******************
...
DevNo		DE	0x02,0x00			; Eeprom DN address start point - DNhi = 0x10 & DNlo = 0x11 
